...
//vec L(float a, vec U, float b, vec V, float s)(return P())
...
pt Neville(pt A, pt B, pt C , pt D, float t) {return N(0,A,1./3,B,2./3,C,1,D,t);}
pt NevilleChordal(pt A, pt B, pt C ,pt D, float t) {
  float ab=d(A,B), bc=d(B,C), cd=d(C,D), d=ab+bc+cd;
  return N(0,A,ab/d,B,(ab+bc)/d,C,1,D,t);} 
pt NevilleCentripet(pt A, pt B, pt C ,pt D, float t) {
  float ab=sqrt(d(A,B)), bc=sqrt(d(B,C)), cd=sqrt(d(C,D)), d=ab+bc+cd;
  return N(0,A,ab/d,B,(ab+bc)/d,C,1,D,t);} 
